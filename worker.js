// SADARI Worker — 결과 공유 링크(API + /r/:id 결과 페이지). 그 외 요청은 정적 자산으로 위임.
  ...
  async fetch(request, env){
    ...
    if (p === "/api/share" && request.method === "POST"){
      try {
        ...
        if (body.length > 16000) return json({ error:"too_large" }, 413);
        ...
        await env.SHARE.put("r:" + id, JSON.stringify(rec)); // 영구 보관
...
<style>
*{box-sizing:border-box}body{margin:0;font-family:"Pretendard","Segoe UI",-apple-system,sans-serif;background:radial-gradient(1200px 600px at 50% -10%,#1e293b,#0f172a);color:#e2e8f0;min-height:100vh;padding:30px 18px 60px}
...
footer{max-width:560px;margin:22px auto 0;text-align:center;color:#94a3b8;font-size:12px}
footer a{color:#38bdf8}
</style></head><body>
<div class="wrap">
<a class="brand" href="/">🪜 SADARI</a>
<p class="tagline">Climb your luck.</p>
<div class="card">
<h1>${who}🪜 사다리타기 결과</h1>
